...
class Framework(object):
    ...
    def train(self, model_pattern):
        ...
        # training data 对训练集数据进行处理
        train_data_loader = data_loader.get_loader(self.config, prefix = self.config.train_prefix, num_workers = 2)
        # dev data 对测试集数据进行处理
        val_data_loader = data_loader.get_loader(self.config, prefix=self.config.dev_prefix, is_test=True)
        ...
        for epoch in range(self.config.max_epoch):
            ...
            while data is not None:
                ...
                '''
                通过model(data)的调用,输入数据data会被传递给模型的forward函数,然后模型会根据输入数据进行计算和预测。
            ...
            precision, recall, f1_score = self.test(val_data_loader, model, current_f1=best_f1_score, output=self.config.result_save_name)
            ...
            if f1_score > best_f1_score:
                ...
                best_recall = recall
                self.logging("saving the model, epoch: {:3d}, precision: {:4.3f}, recall: {:4.3f}, f1: {:4.3f}, best f1: {:4.3f}".
                            format(best_epoch, best_precision, best_recall, f1_score, best_f1_score))
                # save the best model
                path = os.path.join(self.config.checkpoint_dir, self.config.model_save_name)
                if not self.config.debug:
                    torch.save(ori_model.state_dict(), path)

            prec, rec, f1_s = self.test(test_data_loader, model, current_f1=0, output=True)
            print("test f1: {:4.4f}, precision: {:4.4f}, recall: {:4.4f}".format(f1_s, prec, rec))
